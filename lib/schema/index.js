...
import {
  serviceSchema, procedureSchema, conditionSchema, packageSchema,
...
export function generatePageSchemas(config, { pageType, pageData, meta, video } = {}) {
  ...
  // Home reads the clinic-level FAQ; every other page type reads its own pageData
  const hasFaq = !!(
    (pageType === "home" && config.clinic.faq?.length) ||
    (pageType !== "home" && pageData?.faq?.length)
  )
  ...
  switch (pageType) {
    ...
    case "package":
      nodes.push(packageSchema(config, pageData))
      if (pageData?.faq?.length)
        nodes.push(faqSchema(config, path, pageData.faq, ID.package(pageData.slug)))
      break

    default:
      // Page types with no entity node (collection, about, contact, page, …)
      // still get FAQPage when the page supplies one
      if (pageData?.faq?.length)
        nodes.push(faqSchema(config, path, pageData.faq, aboutId))
      break
  }
...
function resolveAboutId(ID, pageType, pageData) {
  switch (pageType) {
    ...
    case "package":   return pageData?.slug ? ID.package(pageData.slug)   : ID.clinic
