...
export function buildSchemaConfig(cfg) {
  ...
  return {
    ...
    clinic: {
      ...
      imageWidth:     1200,
      imageHeight:    800,
      // Only build "Name -- Tagline" when a real tagline exists — leaving this empty when
      // tagline is unfilled lets coreSchemas.js's generated fallback (specialty + city) kick
      // in instead of a dangling "Name -- " with nothing after the dashes.
      description:    clinic.tagline ? `${clinic.name} -- ${clinic.tagline}` : '',
      email:          clinic.email,
      ...
      geo: {
        // transform.ts stores geo as { lat, lng } (strings) — read those too,
        // as Numbers, so the extracted coordinates actually reach the schema.
      ...
      social: {
        ...
        youtube:   clinic.social?.youtube   || '',
      },
      rating: {
        value: parseFloat(d.reviewSummary?.score) || 0,
        // Prefer a real manually-entered total (often higher than what's individually
        // transcribed, e.g. 47 real Google reviews vs 5 shown) — fall back to the actual
        // count of reviews present in d.reviews rather than defaulting to an invalid 0.
        count: parseInt(d.reviewSummary?.count) || (d.reviews || []).length || 0,
        best:  5,
        worst: 1,
      },
