...
// priceRange comes pre-formatted for display (e.g. "₹24,998" or "₹15,000 - ₹25,000") —
// this strips currency symbols/commas to get clean numeric values for schema.org,
// and picks AggregateOffer over a single Offer when the value is actually a range.
export function packageSchema(config, pkg) {
  ...
  const raw = (pkg.priceRange || pkg.price || "").replace(/[₹,\s]/g, "")
  ...
  if (rangeMatch) {
    offers = {
      "@type":         "AggregateOffer",
      "lowPrice":      rangeMatch[1],
      "highPrice":     rangeMatch[2],
