...
// schema.org MedicalSpecialty is a fixed enumeration. Emit the proper
// https://schema.org/{value} URI only when the value is a real enum member;
// otherwise pass it through as plain text, so a display name ("Orthopedic
// Surgeon"), a blank field, or a specialty with no enum match (ophthalmology)
// never produces a broken URI like "https://schema.org/Orthopedic Surgeon".
const MEDICAL_SPECIALTIES = new Set([
  ...
  "Oncologic", "Optometric", "Otolaryngologic", "Pathology", "Pediatric",
  "PharmacySpecialty", "Physiotherapy", "PlasticSurgery", "Podiatric",
  "PrimaryCare", "Psychiatric", "PublicHealth", "Pulmonary", "Radiography",
  "Renal", "RespiratoryTherapy", "Rheumatologic", "SpeechPathology",
  "Surgical", "Toxicologic", "Urologic",
])
function medicalSpecialtyValue(specialty) {
  const v = (specialty || "").trim()
  return MEDICAL_SPECIALTIES.has(v) ? `https://schema.org/${v}` : v
}
...
export function clinicSchema(config) {
  ...
  return {
    ...
    "image": {
      ...
      "@id":     ID.clinicImage,
      "url":     clinic.image,
      "caption": clinic.city ? `${clinic.name}, ${clinic.city}` : clinic.name,
    ...
    "medicalSpecialty":   medicalSpecialtyValue(clinic.specialty),
    ...
    "address": {
      "@type":           "PostalAddress",
      "streetAddress":   clinic.address?.street  || clinic.street  || '',
      "addressLocality": clinic.address?.city    || clinic.city    || '',
      "addressRegion":   clinic.address?.state   || clinic.state   || '',
      "postalCode":      clinic.address?.pincode || clinic.pincode || '',
      "addressCountry":  clinic.address?.country || 'IN',
    },
...
// ── 3b. Team members (additional staff beyond the primary Physician) ────────
// Emits one Person node per non-lead team member — the lead is the primary
// doctor, already in the core graph as the Physician node. Members may be
// non-doctors (nurses, technicians), so the honest generic type is Person.
export function teamSchema(config, members = []) {
  const ID   = ids(config.site.url, config.site.blogPath)
  const base = config.site.url
  return members
    .filter(m => m && m.name && !m.isLead)
    .map((m, i) => ({
      "@type":    "Person",
      "@id":      `${base}/team/#person-${i + 1}`,
      "name":     m.name,
      ...(m.designation && { "jobTitle": m.designation }),
      "worksFor": { "@id": ID.clinic },
      ...(m.qualifications?.length && {
        "hasCredential": m.qualifications.map(q => ({
          "@type": "EducationalOccupationalCredential",
          "name":  q,
        })),
      }),
