...
// ── 4. WebPage ─────────────────────────────────────────────────────────────
// pageType → schema.org @type; anything unlisted gets generic WebPage
const WEBPAGE_TYPE = {
  condition:  "MedicalWebPage",
  procedure:  "MedicalWebPage",
  collection: "CollectionPage",
  about:      "AboutPage",
  contact:    "ContactPage",
}

export function webpageSchema(config, { path, name, description, aboutId, image, datePublished, dateModified, hasFaq = false, pageType }) {
  const ID = ids(config.site.url, config.site.blogPath)
  return {
    "@type":       WEBPAGE_TYPE[pageType] || "WebPage",
    "@id":         ID.webpage(path),
